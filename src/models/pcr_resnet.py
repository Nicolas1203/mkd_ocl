...
class PCRResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes, nf, bias, dim_in=512, **kwargs):
        super().__init__()
        self.in_planes = nf
...
def PCRResNet18(n_classes, dim_in=512, nf=64, bias=True):
    return PCRResNet(BasicBlock, [2, 2, 2, 2], n_classes, nf, bias, dim_in=dim_in)

def PCRResNet18_ImageNet(n_classes, dim_in=512, nf=64, bias=True):
    return ImageNet_PCRResNet(BasicBlock, [2, 2, 2, 2], n_classes, nf, bias, dim_in=dim_in)

class ImageNet_PCRResNet(nn.Module):
